...
int main(int argc, char *argv[])
{
	...
	if (strcmp(serverType, "Decrypt") == 0){
	...
	if (charsWritten < strlen(key)) 
		printf("CLIENT: WARNING: Not all data written to socket!\n");
	// printf("CLIENT: Sent key to server\n");

	// charsWritten = 0;
	// charsWritten = send(socketFD, message, mSize, 0); // Write to the server
	// if (charsWritten < 0) 
	// 	error("CLIENT: ERROR writing to socket");
