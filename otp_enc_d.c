...
int encrypt(char* message, char* key){
	...
	for (i = 0; i < strlen(message); i++){
		offset = i % strlen(key);
		int enc = (mIndeces[i] + kIndeces[offset]) % 27;// Add the two letters and mod the result
	...
	while (1) {
		...
		if (establishedConnectionFD){
			...
			if (newPid < 0){
				...
				// Let the client know what server this is; client will quit as needed
				...
				encrypt(message, key);
				// printf("SERVER: I have encrypted the message and the result is \"%s\"\n", message);
